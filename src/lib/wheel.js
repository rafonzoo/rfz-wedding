 ...
 * @class
 */
export default class iOSPicker {
  /**
   *
   * @param {Object} options
   * @param {string} [options.el]
   * @param {'infinite' | 'normal'} [options.type]
   * @param {number} [options.count]
   * @param {number} [options.sensitivity]
   * @param {({ value: string, text: string }[])} options.source
   * @param {string | null} [options.value]
   * @param {function({ value: string }): void} [options.onChange]
   */
  constructor(options) {
    let defaults = {
      el: '', // dom
    ...
    for (let i = 0; i < source.length; i++) {
      circleListHTML += `<li class="select-option"
                    style="
                      ...
                      transform: rotateX(${-this.itemAngle * i
      ...
      for (let i = 0; i < this.quarterCount; i++) {
        ...
        circleListHTML =
          `<li class="select-option"
                      style="
                        ...
                        height: ${this.itemHeight}px;
                        line-height: ${this.itemHeight}px;
                        transform: rotateX(${this.itemAngle * (i + 1)
          }deg) translate3d(0, 0, ${this.radius}px);
                      "
                      data-index="${-i - 1}"
        ...
        circleListHTML += `<li class="select-option"
                      style="
                        ...
                        height: ${this.itemHeight}px;
                        line-height: ${this.itemHeight}px;
                        transform: rotateX(${-this.itemAngle * (i + sourceLength)
          }deg) translate3d(0, 0, ${this.radius}px);
                      "
  ...
  _moveTo(scroll) {
    ...
    this.elems.highlightList.style.transform = `translate3d(0, ${-scroll * this.itemHeight
      }px, 0)`
